...
Data were extracted from [Yost et al. 2019](https://www.nature.com/articles/s41591-019-0522-3).
```{r}
# Load data
...
We can see that in Yost et al. dataset, CD8.TPEX can be found with profiles matching the reference. TPEX are found scattered all over the original UMAP space. This is because activation signals and other confounding factors were contributing to defining UMAP space. 

This is especially true for the **CD8_act** cluster, where we have many cell types projected. Let's confirm that we can retrieve our reference clusters on what seems to be a highly heterogeneous subset.

# Subset example : **CD8_act** cluster

This cluster is composed of cells driven by their activation signal. One benefit of projection is to overcome these very strong gene programs and focus on the underlying cellular identity.
