...
```{r}
ref.cd8 <- readRDS(file="~/Dropbox/CSI/Datasets/projectTils/NickB.Human_CD8_TILs_atlas_downsampled.rds")
...
query.projected <- Run.ProjecTILs(query.object, ref = ref.cd8, filter.cells = T, ncores = 1)
...
ref <- readRDS(file="~/Dropbox/CSI/Datasets/projectTils/NickB.Human_CD8_TILs_atlas_downsampled.rds")
...
# Projection
gueguen.cd3 <- Run.ProjecTILs(gueguen.cd3, ref = ref.cd8, filter.cells = T, split.by = 'orig.ident', ncores = 6)
table(gueguen.cd3$functional.cluster)
DimPlot(gueguen.cd3, group.by = 'functional.cluster', order = T, cols = cols)
...
# Projection
gueguen.cd3.blood <- ProjecTILs.classifier(gueguen.cd3.blood, ref = ref.cd8, filter.cells = T, split.by = "orig.ident", skip.normalize = T, ncores = 6)

# Full projection
gueguen.cd3.blood <- Run.ProjecTILs(gueguen.cd3.blood, ref = ref.cd8, filter.cells = T, split.by = "orig.ident", skip.normalize = T, ncores = 6)

# Dimplot
DimPlot(gueguen.cd3.blood, group.by = 'functional.cluster', label = T, repel = T, cols = ref.cd8@misc$atlas.palette)

# Radar plots
plot.states.radar(ref = ref.cd8, query = gueguen.cd3.blood, genes4radar = genes)
...
ref.cd8 <- readRDS(file = '~/Dropbox/CSI/Datasets/projectTils/CD8T_human_ref_v0.2_ds.rds')

# Mapping
query.projected <- Run.ProjecTILs(niborski.cd8, skip.normalize = T, ref = ref.cd8, filter.cells = F, ncores = 6, query.assay = 'RNA')
plot.projection(ref.cd8, query.projected)
...
# Radar plots
genes <-  c('SELL','LEF1', "TCF7",'IL7R', "CCR7","CXCR5","GZMK","KLRG1","CX3CR1","KLRG1","FGFBP2",'FCGR3A',"ZNF683","ITGAE","ITGA1",'XCL1',"XCL2",'CD200', "CRTAM",'PDCD1', "HAVCR2","ENTPD1","TIGIT","LAG3","CTLA4","CCL5", "GZMB","CCL5","NKG7","PRF1")

p <- plot.states.radar(ref = ref.cd8, query = query.list, min.cells = 20, genes4radar = genes, return = T)
wrap_plots(p) + theme_bw()

# Project onto original UMAP
niborski.cd8 <- ProjecTILs.classifier(niborski.cd8, skip.normalize = T, ref = ref.cd8, filter.cells = F, ncores = 6, query.assay = 'RNA')
DimPlot(niborski.cd8, group.by = "functional.cluster", label = T, cols = mycols)
```
...
data.seurat <- readRDS(file = "input/Bassez_breast_data/Bassez.seurat.rds")

# Mapping
models <- scGate::get_scGateDB()
query.projected <- Run.ProjecTILs(data.seurat, split.by =  "patient_id", ref.cd8, filter.cells = T,  ncores = 6)
...
# Radar plots
p <- plot.states.radar(ref.cd8, query = query.sub.byExp, min.cells = 10, genes4radar = c('LEF1', "TCF7", "CCR7", "GZMK", "FGFBP2",'FCGR3A','ZNF683','ITGAE', "CRTAM", "CD200",'GNG4', "HAVCR2", "TOX", "ENTPD1", 'SLC4A10','TRAV1-2'),return = T) 
wrap_plots(p) + theme_bw()

# Barplots
n_e <- length(table(query.sub.byExp$E$patient_id))
n_ne <- length(table(query.sub.byExp$NE$patient_id))
a <- plot.statepred.composition(ref.cd8, query.sub.byExp$E, metric = "percent") + ylim(0,
    40) + ggtitle(paste0("Responders; n=", n_e)) + theme_bw()
b <- plot.projection(ref.cd8, query.sub.byExp$E, linesize = 0.5) + ggtitle("Responders")

c <- plot.statepred.composition(ref.cd8, query.sub.byExp$NE, metric = "percent") + ylim(0,
    40) + ggtitle(paste0("Non-Responders; n=", n_ne)) + theme_bw()
d <- plot.projection(ref.cd8, query.sub.byExp$NE, linesize = 0.5) + ggtitle("Non-Responders")
(a | b) /(c | d)
...
```{r}
# Load data
cached.object <- "input/Yost/Yost.seurat.rds"
if (!file.exists(cached.object)) {
    library(GEOquery)
    geo_acc <- "GSE123813"
    datadir <- "input/Yost"
    ...
    query.object <- readRDS(file = cached.object)
}
...
# Mapping
query.projected <- Run.ProjecTILs(query.object.filtered, split.by =  "patient", ref.cd8, filter.cells = T, ncores = 6)
...
# Radar plots
p <- plot.states.radar(ref.cd8, query = query.sub.byTT, min.cells = 30, genes4radar = c('LEF1',"IL7R", "TCF7","LMNA", "CCR7","GZMK", "FGFBP2",'FCGR3A','KLRG1', "CRTAM", "CD200",'GNG4', "HAVCR2", "TOX", "ENTPD1", 'SLC4A10','TRAV1-2'), return = T)
wrap_plots(p) + theme_bw()

# Barplots
n_pre <- length(table(query.sub.byTT$pre$patient))
n_post <- length(table(query.sub.byTT$post$patient))
c <- plot.statepred.composition(ref.cd8, query.sub.byTT$pre, metric = "percent") + ylim(0,
    55) + ggtitle(paste0("Pre treatment; n=", n_pre)) + theme_bw() + RotatedAxis()
d <- plot.statepred.composition(ref.cd8, query.sub.byTT$post, metric = "percent") + ylim(0,
    55) + ggtitle(paste0("Post-treatment; n=", n_post)) + theme_bw() + RotatedAxis()
c | d
...
DefaultAssay(car.cd8) <- "RNA"

# Project CD8s with classifier
car.cd8 <- ProjecTILs.classifier(car.cd8, ref = ref.cd8,  filter.cells = T, ncores = 1)
table(car.cd8$functional.cluster)
DimPlot(car.cd8, group.by = 'functional.cluster', cols = ref.cd8@misc$atlas.palette, label = T, order = T)

# Project CD8s
car.cd8 <- Run.ProjecTILs(car.cd8, ref = ref.cd8,  filter.cells = T, skip.normalize = T, ncores = 6)
table(car.cd8$functional.cluster)
DimPlot(car.cd8, group.by = 'functional.cluster', cols = ref.cd8@misc$atlas.palette, label = T, order = T)

# Radar plots CD8
car.cd8.list <- SplitObject(car.cd8, split.by = "genotype_timepoint")
p <- plot.states.radar(ref.cd8, query = car.cd8.list, genes4radar = c('LEF1', "TCF7", "CCR7", "GZMK", "FGFBP2",'FCGR3A','KLRG1',"PDCD1",'ZNF683','ITGAE', "CRTAM", "CD200","GNG4","CRTAM","XCL1", "PKM","CXCL13", "HAVCR2","GZMB","PRF1","GZMA","TOX", "ENTPD1", 'SLC4A10','TRAV1-2'), return = T)
wrap_plots(p) + theme_bw()

# Barplot CD8
pll <- list()
pll[[1]] <- plot.statepred.composition(ref.cd8, car.cd8.list[["Mock D8"]], metric = "Percent") +
    ggtitle("D8 Mock") + ylim(0, 80)+ theme_bw()+ RotatedAxis()
pll[[2]] <- plot.statepred.composition(ref.cd8, car.cd8.list[["gSUV D8"]], metric = "Percent") +
    ggtitle("D8 KO") + ylim(0, 80) + theme_bw() + RotatedAxis()
pll[[3]] <- plot.statepred.composition(ref.cd8, car.cd8.list[["Mock D28"]],  metric = "Percent") +
    ggtitle("D28 Mock") + ylim(0, 80)+ theme_bw()+ RotatedAxis()
pll[[4]] <- plot.statepred.composition(ref.cd8,  car.cd8.list[["gSUV D28"]], metric = "Percent") +
    ggtitle("D28 KO") + ylim(0, 80) + theme_bw() + RotatedAxis()
grid.arrange(grobs = pll, ncol = 2, nrow = 2, widths = c(1, 1))


# CD4 ------------------------------------
ref.cd4 <- readRDS(file="~/Dropbox/CSI/reference_atlases/CD4TIL_human_ss_0.1.rds")
...
# Project CD4s
car.cd4 <- ProjecTILs.classifier(car.cd4, ref = ref.cd4, split.by = "orig.ident", filter.cells = T, ncores = 1)
...
```{r}
plot.states.radar(ref.cd8, query = car.cd8, min.cells = 30, genes4radar = c('LEF1', "TCF7", "CCR7", "GZMK", "FGFBP2",'FCGR3A','ZNF683','ITGAE', "CRTAM", "CD200",'GNG4', "HAVCR2", "TOX", "ENTPD1", 'SLC4A10','TRAV1-2'))
```
...
# Radars UM1
plot.states.radar(ref.cd8, query = UM1, min.cells = 30, genes4radar = c('LEF1', "TCF7", "CCR7", "GZMK", "FGFBP2",'FCGR3A','ZNF683','ITGAE', "CRTAM", "CD200",'GNG4', "HAVCR2", "TOX", "ENTPD1", 'SLC4A10','TRAV1-2'))

# Radars UM2_3
plot.states.radar(ref.cd8, query = UM2_3,  min.cells = 30, genes4radar = c('LEF1', "TCF7", "CCR7", "GZMK", "FGFBP2",'FCGR3A','ZNF683','ITGAE', "CRTAM", "CD200",'GNG4', "HAVCR2", "TOX", "ENTPD1", 'SLC4A10','TRAV1-2'))
```
