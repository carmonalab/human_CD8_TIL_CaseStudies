...
This dataset contains single-cell data from pre- versus on-treatment biopsies from patients receiving anti-PD1 immunotherapy (n=29) or chemotherapy (n=11). 
TCR-seq data are also available. Lacking an objective measure of response to therapy, in this study they use as a proxy the level of T cell expansion to infer
response to treatment. 

Paper: https://www.nature.com/articles/s41591-021-01323-8
Dataset: https://biokey.lambrechtslab.org/en/data-access
Internal link: https://drive.switch.ch/index.php/s/uTrKHnTMohuUori
...
```{r}
#TCR metadata
tcr1 <- read.csv(sprintf("%s/1879-BIOKEY_barcodes_vdj_combined_cohort1.csv", ddir)) 
tcr1$barcode <- paste0(tcr1$barcode, "-1")
rownames(tcr1) <- tcr1$barcode

data.c1 <- AddMetaData(data.c1, metadata = tcr1)

table(data.c1$SAMPLE_ID, data.c1$patient_id)
#Note: some patients (e.g. BIOKEY_13 do not have TCR_seq data)

#Calculate clomnotype counts and frequencies

clonotypes <- table(data.c1$clonotype)
clonotypes.vs.sample <-  as.matrix(table(data.c1$clonotype, data.c1$SAMPLE_ID))
clonotypes.sums <- apply(clonotypes.vs.sample, 2, sum)

freqs <- vector()

for (i in 1:length(clonotypes)) {
   ctype <- names(clonotypes)[i]
   count <- clonotypes[i]
   sample <- gsub("(\\S+)_clonotype\\S+", "\\1", ctype, perl=T)
   freqs[ctype] <- count/clonotypes.sums[sample]
}

ctypes.vector <- data.c1@meta.data[,c("clonotype")]
freqs.order <- rep(NA,length(ctypes.vector))
counts.order <- rep(NA,length(ctypes.vector))

for (r in 1:length(ctypes.vector)) {
   c <- ctypes.vector[r]
   if (!is.na(c)) {
      counts.order[r] <- clonotypes[c]
      freqs.order[r] <- freqs[c]
   }
}
data.c1$clone_count <- counts.order
data.c1$clone_freq <- freqs.order

hist(data.c1$clone_count)
hist(data.c1$clone_freq)

#Define levels of expansion
data.c1$expand <- NA
data.c1$expand[!is.na(data.c1$clone_count) & data.c1$clone_count==1] <- "Single"
data.c1$expand[!is.na(data.c1$clone_count) & data.c1$clone_count>1 & data.c1$clone_count<=5] <- "Small(1-5)"
data.c1$expand[!is.na(data.c1$clone_count) & data.c1$clone_count>5 & data.c1$clone_count<=10] <- "Medium(6-10)"
data.c1$expand[!is.na(data.c1$clone_count) & data.c1$clone_count>11 & data.c1$clone_count<=20] <- "Large(11-20)"
data.c1$expand[!is.na(data.c1$clone_count) & data.c1$clone_count>20] <- "Hyper(>20)"

data.c1$expand <- factor(data.c1$expand, 
                levels = c("Hyper(>20)", "Large(11-20)", "Medium(6-10)", "Small(1-5)", "Single", NA))

table(data.c1$expand, useNA = "ifany")
```
